locals {
  ...
  landscape_private_keyvault_name = format("%s%s%sprvt%s", replace(local.landscape_env_verified, "/[^A-Za-z0-9]/", ""), local.location_short, local.random_id_verified)
  landscape_user_keyvault_name    = format("%s%s%suser%s", replace(local.landscape_env_verified, "/[^A-Za-z0-9]/", ""), local.location_short, local.random_id_verified)
